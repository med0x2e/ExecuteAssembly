...
namespace PEModuleHelper {
	...
	void StompPEHeaders(HANDLE hProcess) {
		...
		printf("[+]: Stomping 9 PE DOS headers:\n");
		fflush(stdout);
	...
	void StompPEDOSHeader(LPSTR ntheaderAddr)
	{
		...
		DWORD Protect;
		//zeroing out 224 bytes of the PE Header leads to crashing the loaded .net assembly, I'm only patching specific PE DOS HEADER offsets/signatured byte sequences as a workaround.
		SIZE_T Size = 2;
